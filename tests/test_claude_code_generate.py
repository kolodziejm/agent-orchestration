...
def load_generate_module():
    spec = importlib.util.spec_from_file_location("claude_code_generate", GENERATE)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Cannot load generate module from {GENERATE}")
...
def parse_frontmatter_tools(content: str) -> list:
    ...
    single-line comma-separated form the generator emits.
    """
...
class ClaudeCodeGenerateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_generator_produces_every_role_agent_with_expected_frontmatter(self):
    ...
    def test_generator_refuses_symlinked_output_path_and_leaves_target_untouched(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            result = subprocess.run(
                [sys.executable, str(GENERATE), "--output", str(symlinked_output)],
                cwd=ROOT,
                text=True,
                capture_output=True,
            )
    ...
    def test_generator_refuses_symlinked_default_output_and_leaves_target_untouched(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            (repo / "build" / "claude-code").symlink_to(target)

            result = subprocess.run(
                [sys.executable, str(repo / "harnesses" / "claude-code" / "generate.py")],
                cwd=repo,
                text=True,
                capture_output=True,
            )
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("Refusing symlinked output path", result.stderr)
            self.assertTrue((repo / "build" / "claude-code").is_symlink())
            self.assertEqual(marker.read_text(), "original")
    ...
    def test_generator_rejects_a_typoed_harness_value_instead_of_silently_ignoring_the_profile(self):
        with tempfile.TemporaryDirectory() as directory:
            repo = build_temp_repo(Path(directory) / "repo")
            ...
            result = subprocess.run(
                [sys.executable, str(repo / "harnesses" / "claude-code" / "generate.py"), "--output", str(Path(directory) / "out")],
                text=True,
                capture_output=True,
            )
            self.assertNotEqual(result.returncode, 0)
    ...
    def test_generator_fails_instead_of_generation_with_zero_matching_profiles(self):
        with tempfile.TemporaryDirectory() as directory:
            repo = build_temp_repo(Path(directory) / "repo")
    ...
    def test_opencode_generator_ignores_claude_code_profile(self):
        opencode_generate = ROOT / "harnesses" / "opencode" / "generate.py"
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory)
            subprocess.run(
                [sys.executable, str(opencode_generate), "--output", str(output)],
                cwd=ROOT,
                check=True,
            )
            manifest = json.loads((output / "manifest.json").read_text())
    ...
    def test_frontmatter_scalar_values_are_json_quoted_and_survive_special_characters(self):
        ...
        self.assertIn(name_line, generated)
        self.assertIn(description_line, generated)
    ...
    def test_generator_rejects_bash_deny_without_granting_shell(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [sys.executable, str(repo / "harnesses" / "claude-code" / "generate.py"), "--output", str(output)],
                cwd=repo,
                text=True,
                capture_output=True,
            )
    ...
    def test_require_subagent_mode_rejects_a_non_subagent_role(self):
        ...
        with self.assertRaisesRegex(SystemExit, "mode"):
            generate_module.require_subagent_mode("worker", {"mode": "primary"})
    ...
    def test_frontmatter_round_trips_through_pyyaml(self):
        ...
        generated = generate_module.frontmatter("tricky-role", config, model_config, native_vision=False)
        block = generated.split("---", 2)[1]
        parsed = yaml.safe_load(block)
        self.assertEqual(parsed["name"], "tricky-role")
        self.assertEqual(parsed["description"], tricky_description)

    def test_generator_produces_pyyaml_parseable_frontmatter_for_every_role(self):
        if yaml is None:

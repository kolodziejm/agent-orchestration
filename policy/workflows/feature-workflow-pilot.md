...
Resolve the canonical UX readiness gate before planning implementation slices. When a large or uncertain feature creates or materially changes user-facing UI behavior without an approved design, explicit interaction specification, or unambiguous established product pattern, iterate with `design-partner` and the user over a disposable temporary prototype. Planning may begin only after the user freezes the design; the planner handoff then carries the frozen design decisions and temporary prototype reference, not a new repository prototype artifact.

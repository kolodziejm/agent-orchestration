...
- Every delegation MUST state a logical stopping condition: the terminal completion, blocker, or checkpoint condition at which the lane returns control.
- Every delegation MUST also have an enforceable whole-lane execution cap that bounds the lane's total execution and forces control back to orchestration when the cap is reached. Use the strongest supported cap, such as a task-calibrated turn limit or a cancellable whole-lane runtime deadline. Reaching the cap initiates hard cancellation rather than fabricating terminal completion. After acknowledged cancellation, return partial progress or `BLOCKED`. If the operation independently reaches another terminal state first, report that actual terminal result rather than relabeling it `BLOCKED`; an unacknowledged cancellation remains non-terminal and must be reported as such. The deadline never self-extends.
...
- Delegated `validator` or `debugger` lanes MAY monitor CI, deployment, security, dependency, or other external-job state when the handoff names the whole-lane deadline, the poll interval, and the per-call timeout, and hard cancellation of the lane and its in-flight process/tool call is available. Terminal success maps to `PASS`, terminal unsuccessful maps to `FAIL`; a job still non-terminal at the lane deadline maps to `BLOCKED` with last progress time and evidence.
- When the harness cannot guarantee hard cancellation of a delegated lane, external-job waiting stays in the primary behind a bounded process/tool timeout that terminates the underlying operation at the deadline, and delegated agents make at most one one-shot status query (non-terminal → `BLOCKED`). A harness may tighten this in its own operational note.
- Prompt deadlines, turn limits/`max_turns`, and steering do not terminate in-flight operations and MUST NOT be treated as cancellation.
- Independently of the logical stopping condition and whole-lane execution cap, every potentially blocking tool invocation—including shell, test/build, Docker, network, browser/device, or external-job monitoring—MUST have an enforceable per-call deadline or timeout. Turn limits such as max_turns do not bound the duration of an individual tool call and are insufficient on their own; a per-call timeout likewise does not bound the whole lane.
...
- On expiry, terminate/cancel the underlying operation when supported. Return `BLOCKED` with last progress/evidence/prerequisite only after cancellation acknowledgement. If the operation reaches another confirmed terminal state first, report its actual terminal result. Without either confirmation, report the breach or watchdog failure and keep the lane non-terminal. A steering message or unacknowledged request to stop is not equivalent to termination.
- When a deadline breach leaves an agent non-terminal and the harness exposes only a human UI stop/abort control, the primary MUST immediately report the breach and give the exact harness-native manual stop steps; it MUST continue treating the agent/task as non-terminal until the stop or a terminal state is confirmed. A manual UI fallback is recovery only and does not satisfy the hard-cancellation guarantee required before launch.
...
- Where the harness exposes both child progress events and targeted hard cancellation, the primary MUST arm independent watchdog boundaries: a startup deadline before first meaningful progress, an idle deadline since the last meaningful state change, and a total-runtime deadline regardless of continued progress. Merely creating, queueing, or starting a child is not meaningful progress. Only an acknowledged hard cancellation may produce terminal `BLOCKED`; its evidence MUST include the last meaningful progress time and kind. A failed or missing cancellation acknowledgement leaves the lane non-terminal and under monitoring, with the failure reported rather than silently disabling the watchdog.
- A watchdog-owned cancellation retry after expiry is a safety control, not an extension of the child lane or permission to resume or duplicate its work. It MAY continue at a bounded cadence until the child becomes terminal or the watchdog/host shuts down, but it MUST surface the failed or missing acknowledgement, remain cancellable on shutdown, preserve the original deadline, and never claim terminal state prematurely.
...
- Only the orchestrator may authorize source implementation or repository persistence. Planner managed outputs become repository artifacts only through one selected worker after the applicable approval. A design-partner prototype remains a temporary artifact outside the repository; after design freeze, only its authoritative decisions and implementation handoff may flow into approved repository artifacts.
...
Before production planning or implementation, the orchestrator must classify UX readiness for any scope that creates or materially changes user-facing UI behavior. The UX readiness gate applies when meaningful decisions about the journey, states, transitions, hierarchy, or interaction behavior remain unresolved because there is no approved design, explicit interaction specification, or unambiguous established product pattern. It does not apply to copy edits, straightforward visual fixes, or faithful implementation of an approved design or clear existing pattern.

When the gate applies, route the unresolved UX scope to `design-partner`. Keep the design process human-in-the-loop: establish the target user, task, platform, flow, states, interactions, meaningful alternatives, trade-offs, and recommendation; expose a lightweight clickable HTML/CSS/JS prototype as a temporary harness-managed artifact outside production source and version control; and reuse the same design-partner task for user feedback instead of restarting design context. The temporary artifact is a disposable decision instrument, not repository output or production code. It must never silently fall back to a repository path. If the active harness cannot expose the temporary artifact, return `BLOCKED` with the exact missing prerequisite rather than inventing an artifact mechanism or writing the prototype into the repository.

Do not delegate production planning or implementation for the unresolved UI scope until the user explicitly freezes the design. After design freeze, pass the selected journey, state map, interaction rules, edge states, implementation constraints, and temporary prototype reference to `planner` or the selected worker as appropriate. Preserve the authoritative frozen-design handoff; the temporary prototype itself need not be retained. During implementation, validator owns deterministic acceptance against the frozen design.

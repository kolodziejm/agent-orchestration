...
- Every delegation MUST also have an enforceable whole-lane execution cap that bounds the lane's total execution and forces a terminal return when the cap is reached. Use the strongest supported cap, such as a task-calibrated turn limit or a cancellable whole-lane runtime deadline. Reaching the cap returns partial progress or `BLOCKED` and never self-extends.

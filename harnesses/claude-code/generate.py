...
- `bash = "ask"` in policy/routing.toml has no per-subagent enforcement in
  ...
  per agent file. `Bash` is therefore granted to generated roles that request
...
# already-resolved, so a symlink swapped in at "build/claude-code" would
# never hit the is_symlink() check below when --output is omitted.
DEFAULT_OUTPUT = ROOT / "build" / "claude-code"
...
def tools_for(config: dict, native_vision: bool, role: str | None = None) -> str:
    ...
    # in the handoff to each generated role.
    return ", ".join(tools)
...
def require_subagent_mode(role: str, config: dict) -> None:
    ...
    # different mode have no primary-agent equivalent to generate here.
    mode = config.get("mode")
    if mode != "subagent":
        raise SystemExit(
            f"Claude Code harness only generates subagent roles: {role!r} has mode {mode!r}"
        )
...
def generate_into(output: Path) -> None:

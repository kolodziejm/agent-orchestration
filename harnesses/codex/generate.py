...
# already-resolved, so a symlink swapped in at "build/codex" would
# never hit the is_symlink() check below when --output is omitted.
DEFAULT_OUTPUT = ROOT / "build" / "codex"
...
def load_profile(name: str) -> dict:
    ...
    if harness not in {"opencode", "codex"}:
        raise SystemExit(
            f"Profile {name!r} declares harness = {harness!r}; Codex will not generate it"
        )
...
def generate_control_plane(profile: dict) -> str:
    """Generate profile control intent using Codex model and effort names."""
...
def generate(output: Path, profile_name: str = DEFAULT_PROFILE) -> None:
...
if __name__ == "__main__":
    ...
    generate(args.output, args.profile)

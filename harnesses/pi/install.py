...
# but are not generated, copied, validated, or claimed by the current installer.
...
# current generation never copies, validates, or claims them.
...
LEGACY_RETIRED_RUNTIME_EXTENSIONS = frozenset({
    ...
    # retired. They are migration-only installed-manifest claims: current generation
...
def load_and_preflight(generated: Path, target: Path, adopt: bool) -> tuple[dict, dict]:
    current = json.loads((generated / "manifest.json").read_text())
...
def desired_state(
    ...
    current, previous = load_and_preflight(generated, target, adopt)
    files: dict[Path, str] = {}
...
def validate_installed(files: dict[Path, str], target: Path) -> None:
    ...
    # Post-install validation is scoped to the generated bundle and any
...
def install(
    ...
    with tempfile.TemporaryDirectory(prefix="agent-orchestration-") as directory:
        generated = Path(directory) / "pi"
        subprocess.run(
            [sys.executable, str(GENERATE), "--profile", profile, "--output", str(generated)],
            check=True,
        )
        ...
        try:
            ...
            raise SystemExit(f"Malformed generated Pi manifest: {generated_manifest_path}") from error
        validate_manifest(generated_manifest, "generated manifest")

        files, deletions = desired_state(generated, target, adopt)
        manifest_path = target / MANIFEST_NAME
        declared_launchers = generated_manifest.get("launchers", [])
        if resolved_bin is None:
            ...
            for launcher_name in declared_launchers:
                ...
                launcher_source = generated / launcher_name

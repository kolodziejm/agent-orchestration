...
- target each slice at `<=400` human-authored maintained changed lines and `<=12` human-authored maintained changed files;
...
- a proposed `401–800` human-authored changed-line or `13–24` human-authored changed-file slice needs a concrete rationale and explicit user approval before implementation or promotion;
...
- prefer smaller slices when concern, dependency, validation, or review boundaries warrant them.

The plan records expected human-authored, generated-artifact, and lockfile accounting, the rationale and approval status for any above-target slice, affected areas, dependencies, and order. Workers stay inside the assigned slice and stop before an unapproved threshold breach or at the absolute ceiling. Independent implementation may be parallel only in isolated, non-overlapping branches or worktrees; PR or fallback promotion and user checkpoints remain ordered, and parallel lanes must not bypass accounting or serial validation.

After every PR or fallback review unit, the orchestrator presents a bird's-eye checkpoint and waits for explicit user approval before the next PR or review unit. The checkpoint includes purpose/concern, behavior before/after, key decisions and approvals, human-authored diff lines/files, separate generated-artifact and lockfile lines/files, affected areas/files, risks, validation evidence or `not run`/`BLOCKED` status, residual work, and the next proposed PR or fallback unit. Earlier initiative or slice approval never implies approval for the next unit.

...
DeepSeek Flash is used for the primary session, small model, built-in build and
plan intent, and every canonical child role. The profile declares native vision,
so visual work can be handled by the selected model without adding a synthetic
`vision-*` role.
